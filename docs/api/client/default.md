...
| Поле | Тип | Описание |
| --- | --- | --- |
| `parse_mode` | `str \| None` | Режим разметки: `"html"` или `"markdown"` (`ParseMode.HTML`, `ParseMode.MARKDOWN`) |
| `disable_link_preview` | `bool \| None` | Отключить превью ссылок в сообщениях |
| `notify` | `bool \| None` | Отправлять уведомления получателям |
