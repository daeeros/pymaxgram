...
| `adjust(*sizes: int)` | Перераспределить кнопки: `adjust(2, 1)` → 2 кнопки в 1-м ряду, 1 во 2-м |
| `copy()` | Создать копию builder |
| `export() -> list[list[Button]]` | Получить список рядов кнопок |
| `as_markup() -> InlineKeyboardAttachmentRequest` | Получить объект вложения для API |
