...
.. list-table::
   ...
   * - ``type``
     - ``str``
     - Тип чата (``"chat"``, ``"dialog"``, ``"channel"``)
   * - ``status``
     - ``str``
     - Статус (``"active"``, ``"removed"``, ``"left"``, ``"closed"``)
   * - ``title``
     - ``str | None``
     - Название чата
   * - ``icon``
   ...
   * - ``last_event_time``
     - ``int``
     - Время последнего события (Unix timestamp)
   * - ``participants_count``
     - ``int``
