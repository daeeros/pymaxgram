...
| Метод | Описание |
|-------|----------|
| `build_request_path() -> str` | Подставляет параметры в `__api_path__` (например, `{chat_id}` → `123`) |
| `build_query_params() -> dict` | Формирует query-параметры для GET/DELETE запросов |
| `build_request_body() -> dict \| None` | Формирует JSON-тело для POST/PUT/PATCH запросов |
| `emit(bot) -> T` | Выполняет метод через переданный бот |
| `__await__() -> T` | Позволяет `await method` (требует привязанный bot через контекст) |
