...
.. list-table::
   ...
   * - Метод
     - Описание
   * - ``await set_state(state: str | State | None)``
     - Установить текущее состояние
   * - ``await get_state() -> str | None``
     - Получить текущее состояние
   * - ``await set_data(data: dict) -> None``
     - Заменить все данные
   * - ``await get_data() -> dict``
     - Получить все данные
   * - ``await get_value(key, default=None)``
     - Получить одно значение по ключу
   * - ``await update_data(**kwargs) -> dict``
     - Обновить данные (merge) и вернуть результат
   * - ``await clear() -> None``
