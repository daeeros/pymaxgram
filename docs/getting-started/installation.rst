...
.. list-table::
   ...
   * - ``aiohttp``
     - >= 3.9.0, < 3.14
     - Асинхронный HTTP-клиент для запросов к API
   * - ``pydantic``
     - >= 2.4.1, < 2.13
     - Валидация данных и типизированные модели
   * - ``magic-filter``
     - >= 1.0.12, < 1.1
     - Декларативная фильтрация через объект ``F``
   * - ``aiofiles``
     - >= 23.2.1, < 26.0
     - Асинхронное чтение/запись файлов
   * - ``certifi``
     - >= 2023.7.22
     - Корневые SSL-сертификаты
   * - ``typing-extensions``
     - >= 4.7.0, <= 5.0
     - Расширения типизации Python
