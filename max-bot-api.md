...
Объект входящего события. Это базовый класс — реальные события различаются полем `update_type` и дополнительными полями в зависимости от типа.
...
| Поле          | Тип                 | Описание                                              |
| ------------- | ------------------- | ----------------------------------------------------- |
| `update_type` | string              | Тип события (например, `message_created`)             |
| `timestamp`   | int64               | Unix-время наступления события                        |
| `message`     | [`Message`](#message) | Новое сообщение (для `message_created`)             |
| `user_locale` | string, nullable    | Язык пользователя (IETF BCP 47), только в диалогах    |

**Основные типы событий** (значения `update_type`), на которые можно подписаться через `update_types` в `POST /subscriptions`:
...
- и другие (конкретный список может расширяться — см. актуальную документацию).
