...
> С 19 июля адрес `platform-api.max.ru` заменён на `platform-api2.max.ru`. Сертификат
> удостоверяющего центра Минцифры России должен быть добавлен как доверенный — pymaxgram
> поставляет его в комплекте и доверяет по умолчанию.
...
- **Максимальная частота запросов**: 30 rps на `platform-api2.max.ru`. Превышение даёт `429`.
...
Все примеры используют cURL. Базовый URL — `https://platform-api2.max.ru`. Везде подразумевается заголовок `Authorization: {access_token}`.
